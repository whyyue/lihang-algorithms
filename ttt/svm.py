...
class SVM(object):
	...
	def train(self, features, labels):
		...
		for times in range(self.Max_Interation):
			...
			# 公式(7.115) 及 公式(7.116)
			b_new = 0
			b1_new = -E1 - self.Y[i1] * self._K_(self.X[i1], self.X[i1]) * (alpha1_new - self.alpha[i1]) - self.Y[i2] * self._K_(self.X[i2], self.X[i1]) * (alpha2_new - self.alpha[i2]) + self.b
			b2_new = -E2 - self.Y[i1] * self._K_(self.X[i1], self.X[i2]) * (alpha1_new - self.alpha[i1]) - self.Y[i2] * self._K_(self.X[i2], self.X[i2]) * (alpha2_new - self.alpha[i2]) + self.b
...
if __name__ == '__main__':
	...
	print('svm1 the accuracy socre is ', score)
