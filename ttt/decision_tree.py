...
class Tree(object):
	...
	def predict(self, features):
		...
		if self.dict.get(features[self.feature]):
...
def calc_ent_grap(x, y):
	...
	ent_grap = base_ent - calc_condition_ent
	return ent_grap

def recurse_train(trainset, train_label, features, epsilon):
	global total_class
	...
	for feature in features:
		A = np.array(trainset[:, feature].flat)
	...
	feature_col = np.array(trainset[:, max_feature].flat)
	feature_value_list = set([feature_col[i] for i in range(feature_col.shape[0])])
	for feature_value in feature_value_list:

		index = []
		for i in range(len(train_label)):
			if trainset[i][max_feature] == feature_value:
				index.append(i)

		sub_trainset = trainset[index]
		sub_train_label = train_label[index]

		sub_tree = recurse_train(sub_trainset, sub_train_label, sub_features, epsilon)
		tree.add_tree(feature_value, sub_tree)

	return tree

@log
def train(trainset, train_label, features, epsilon):
	return recurse_train(trainset, train_label, features, epsilon)
