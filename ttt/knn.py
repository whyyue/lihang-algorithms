...
def Predict(testset, trainset, train_labels):
	...
	for test_vec in testset:
		...
		for i in range(k, len(train_labels)):
			...
			dist = np.linalg.norm(train_vec - test_vec) # 计算两个点欧式距离
...
if __name__ == '__main__':
	...
	time_3 = time.time()
	print('read data cost', time_3 - time_2, 'second', '\n')

	print('Start predicting')
	test_predict = Predict(test_features, train_features, train_labels)

	time_4 = time.time()
	print('read data cost', time_4 - time_3, 'second', '\n')
